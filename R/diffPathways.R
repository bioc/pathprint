...
# function to return pathawys consistently different between two groups of fingerprints
# fingerprints is a matrix of fingerprints
# The number of columns in fingerprints should correspond to the length of fac
# fac is a vector of levels or characters corresponding to two groups over which to conduct the comparison
	{
	...
	if (length(fac) != ncol(fingerprints)) stop ("fac length must equal nubmer of fingerprints")
	if ((threshold < 0 | threshold > 2)) stop ("threshold value should be between 0 and 2")
