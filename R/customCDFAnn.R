...
function(data, ann)
  # Annotates according to custom cdf, averaging to resolve redundancies
  {
    ...
    if (sum(c("ID", "AFFYID") %in% colnames(ann)) < 1) stop ("annotation frame does not contain expected columns")
...
# convert to numeric as occasionally factors can complicate matters
    data.temp[,-1] <- apply(data.temp[,-1, drop = FALSE], 2, function(x){as.numeric(as.character(x))})

# aggregate Entrez IDs using mean
# edit Nov 27th 2011, changed to median as more appropriate for the human ST chips.
# edit Nov 28th 2011, changed back to mean. Not necessary to make this modification at this stage
    data.agg <- aggregate(data.temp[,-1], list(EntrezID=data.temp$EntrezID), mean)
