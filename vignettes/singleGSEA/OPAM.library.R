   ...
   for (gs.i in 1:N.gs) {
      ...
      if (length(gene.overlap) == 0) { 
         ...
         if (output.score.type == "ES") {
            ...
            OPAM <- OPAM.Projection(data.array = m, gene.names = gene.names, n.cols = Ns, 
                                 n.rows = Ng, weight = 0.25, statistic = "area.under.RES",
                                 gene.set = gene.overlap, nperm = 50) 
...
#     windows(width=15, height=height)
   ...
   MSIG.HeatMapPlot.7(V = V3, row.names = markers,
                      ...
                      col.names = sample.names2, main = main.string, xlab="  ", ylab="  ", row.norm = T,  
                      cmap.type = 3, char.rescale = char.res,  legend=legend)

     V4 <- data.frame(V3)
     colnames(V4) <- sample.names2
     row.names(V4) <- paste(markers, seq(1, length(markers)), sep="_")
     if (!is.null(markers.file)) {
        write.gct(gct.data.frame = V4, descs = model.descs2, filename = markers.file)  
     }
